...
async function generateEmbeddings(texts) {
    ...
    try {
        for (const text of texts) {
            ...
            embeddingsArray.push(response.data.embedding)
        }
    } catch (error) {
...
async function createAndStoreEmbeddings(embeddings) {
    ...
    const items = [
        // { id: 'item_1', values: embeddings[0] },
        // { id: 'item_2', values: embeddings[1] },
...
generateEmbeddings(texts)
    ...
    .then(() => {
        queryPinecone('blah_blah_blah') // embedding values don't matter because everything is hardcoded right now.
    })
